        ...
        try:
            ...
            for selector, val in data.items():
                if isinstance(val, pd.DataFrame):
                    # 1. Create a pristine numeric dataframe
                    safe_df = pd.DataFrame()
                    for col in val.columns:
                        safe_df[col] = pd.to_numeric(val[col], errors='coerce')
                        
                    # 2. Check Orientation: If 'Sim_0' was the index, data is sideways. Flip it.
                    ...
                    # 4. Extract Mean safely into a raw NumPy array
            ...
            df.index = np.arange(len(df)) # Force integer index
            x = df.index.values
            ...
            traceback.print_exc() # This will print the exact line number if it crashes
